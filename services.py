...
def analyze_nutrition(product_info: str) -> str:
    """Send product info to Sarvam AI and return nutritional analysis."""
...
If no relevant product details are found, simply respond with 'no information available.'
    ...
    try:
        ...
        headers = {
            ...
            # include standard Bearer as fallback for openai compatibility endpoints
        ...
        # ── DEBUG RESPONSE ──
        print("DEBUG ► Raw LLM Response Status:", response.status_code)
        print("DEBUG ► Raw LLM Response Body:")
        print(response.text)
        # ── END DEBUG ──
        ...
        if not choices:
            return f"No analysis returned. (Raw output: {response.text})"
            
        content = choices[0].get("message", {}).get("content")
        return content.strip() if content else "No text generated by the model."
